...
REFERENCE_PHRASES = {
    ...
    "Kiki": [
        ...
        "A journey of Rome was not built in a single day.",
...
def collect_segment_embeddings(unlabelled_paths, embedder, segmenter):
    ...
    extra_audio_files = [
        ...
        "bruno.wav",
        "luna.wav",
        "bella_contrastive.wav",
        "bruno_contrastive.wav",
    ]
    turn_paths = (
        sorted(os.path.join("turns", f) for f in os.listdir("turns") if f.endswith(".wav"))
    ...
    print(
        f"  Combined {sum(1 for p in all_paths if os.path.exists(p))} audio files ({len(combined_tts) / TTS_SR:.1f}s total)"
    )
    ...
    print(
        f"  Segmenter found {len(segments)} segments, {len(long_segs)} above {MIN_EMBED_SAMPLES / 16000:.1f}s threshold"
    )
...
def collect_labelled_embeddings(ref_paths, embedder):
    ...
    print(f"  Collected {len(embeddings)} labelled embeddings from {len(label_names)} speakers")
...
def main():
    ...
    print("\n=== Step 3: Generating unlabelled samples for VAE training ===")
    unlabelled_paths = generate_unlabelled_samples(tts, "unlabelled")

    print("\n=== Step 4: Collecting embeddings for VAE training ===")
